...
async def remind(
    ...
    # === 設定完了メッセージ（embedなしで通知に内容が見える形式） ===
    send_text = f"✅ リマインダー設定完了\n🕒 時刻: {format_jst(datetime.datetime.fromtimestamp(remind_ts, datetime.timezone.utc))}\n💬 内容: {message}"
    if role:
        send_text += f"\n🔔 メンション: {role.mention}"
    send_text += f"\n📍 送信先: {'DM' if destination.value=='dm' else f'#{channel.name}'}"
    ...
    try:
        if destination.value == "channel":
            ...
            await channel.send(send_text, allowed_mentions=allowed)
        else:
