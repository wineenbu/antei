...
async def remind(
    ...
    content = f"{role.mention+' ' if role else ''}⏰ {format_jst(dt)} にリマインダー設定完了\n💬 {message}\n📍 {'DM' if destination.value=='dm' else f'#{channel.name}'}"
...
tree.autocomplete("remind", "weekday")(weekday_autocomplete)
